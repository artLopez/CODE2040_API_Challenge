...
public class ISOString {
    ...
    public static void sendHTTPPOST()
    {
    	 try {
            ...
            //post sent with the dictionary {"token" : myToken, "datestamp": finalISO(datestamp}
             post.setEntity(new StringEntity("{\"token\":\"" + getToken() + "\" , \"datestamp\":\"" + finalISO + "\"}", ContentType.create("application/json")));
    ...
    public void makeHTTPPOSTRequest() {
        try {
            ...
            post.setEntity(new StringEntity("{\"token\":\"" + getToken() + "\"}", ContentType.create("application/json")));
